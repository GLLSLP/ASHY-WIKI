...
const getAllCachedNodes = async (cache, getNode) => {
    const dir = (0, exports.cacheDirectory)(cache);
    ...
    return (await Promise.all(files.map((f) => {
        ...
        return (0, exports.getCachedNode)(cache, id, getNode);
    }))).filter(util_1.nonNullable);
};
exports.getAllCachedNodes = getAllCachedNodes;
const setCachedNode = (cache, id, data) => {
    return fs.promises.writeFile(path.join((0, exports.cacheDirectory)(cache), `${encodeURIComponent(id)}.json`), JSON.stringify({
...
const getCachedNode = async (cache, id, getNode) => {
    ...
    if (!node) {
        try {
            ...
            await fs.promises.unlink(path.join((0, exports.cacheDirectory)(cache), `${encodeURIComponent(id)}.json`));
        }
        catch (err) { }
        return undefined;
    }
    try {
        const data = JSON.parse(await fs.promises.readFile(path.join((0, exports.cacheDirectory)(cache), `${encodeURIComponent(id)}.json`), 'utf8'));
...
const setInboundReferences = (cache, data) => {
    return fs.promises.writeFile(path.join((0, exports.cacheDirectory)(cache), inboundFile), JSON.stringify(data));
};
exports.setInboundReferences = setInboundReferences;
const getInboundReferences = async (cache) => {
    try {
        return JSON.parse(await fs.promises.readFile(path.join((0, exports.cacheDirectory)(cache), inboundFile), 'utf8'));
    }
    catch (err) {
        return undefined;
    }
...
const clearInboundReferences = async (cache) => {
    try {
        await fs.promises.unlink(path.join((0, exports.cacheDirectory)(cache), inboundFile));
    }
    catch (e) { }
};
