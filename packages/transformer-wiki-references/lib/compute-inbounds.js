...
async function generateData(cache, getNode) {
    ...
    currentGeneration = Promise.resolve().then(async () => {
        const nodes = await (0, cache_1.getAllCachedNodes)(cache, getNode);
            ...
            let node = nodes.find((x) => {
                ...
                if (filePath) {
                    if (tree) {
                        ...
                        return (0, path_1.basename)(filePath, (0, path_1.extname)(filePath)) === title;
                    }
                }
            });
            return node;
        ...
        await Promise.all(nodes
            ...
            .map((data) => (0, cache_1.setCachedNode)(cache, data.node.id, data)));
        ...
        await (0, cache_1.setInboundReferences)(cache, inboundReferences);
