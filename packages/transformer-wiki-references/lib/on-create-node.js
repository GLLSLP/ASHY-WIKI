...
function getTitle(node, content) {
    ...
    return (0, markdown_utils_1.findTopLevelHeading)(content) || '';
}
...
const onCreateNode = async ({ cache, node, loadNodeContent, getNode }, _options) => {
    ...
    const outboundReferences = (0, get_references_1.getReferences)(content, (ref) => {
        ref.referrerNode = node;
        return ref;
    });
    const title = getTitle(node, content);
    const aliases = getAliases(node);
    await (0, cache_1.clearInboundReferences)(cache);
    await (0, cache_1.setCachedNode)(cache, node.id, {
        node,
        outboundReferences,
        title,
        aliases,
